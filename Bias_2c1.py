...
data_name="Data23/Bias_2c1"
if not os.path.exists("Data/"+data_name):
    os.makedirs("Data/"+data_name)
#Bias_L=np.linspace(0,1,11)  ##Data14
#Bias_L=np.linspace(2.5,4.5,11) ### Data 15
#Bias_L=np.linspace(3.2,3.4,11) ### Data 16
#Bias_L=np.linspace(0.8,1.8,11) ### Data 17
#Bias_L=np.linspace(1.5,2.2,8) ### Data 18
#Bias_L=np.linspace(2,3.5,11) ### Data 19
#Bias_L=np.linspace(0.8,2,13) ### Data 20
Bias_L=np.linspace(0,2.1,8) ### Data 21
Bias_L=np.linspace(0,2,11) ### Data 22
#Bias_L=np.linspace(0.4,2.4,11) ### Data 22 Dterms
Bias_L=np.linspace(0.8,2,11) ### Data 23
chi_1fix=chi_1
chi_2fix=chi_2
...
for Bias in Bias_L:
    ...
    H_clas=Clas_Field(m_1,n_1,Mean_a,1)[1]
...
#用 BrBG colormap 创建一个 color map 对象
...
ax2.scatter(Bias_L,Chern_L,color='r',s=15)
ax2.tick_params(axis='y', colors='r')
ax2.set_xlim(Bias_L[0],Bias_L[-1])
ax2.set_yticks([0,1])
ax2.invert_xaxis()
plt.savefig("./Data/"+data_name+'/Bias_2c1.eps')
plt.show()
